...
| `libero_object_packing` | 10 × 50 | Multi-item packing variant of `libero_object_all_variance` — every grocery item on the floor must end up in the basket under the **combined** variance distribution (position + permutation + basket-swap), with a radius-aware 5cm clearance pass (`enforce_clearance.py`) re-separating each layout so objects never start overlapping. Uses the stateful `pack_all_into` predicate (delivered items teleport to a far graveyard pose so they cannot regress; success when all 6 delivered). Reports both `success` (binary) and `completion_rate` (`delivered/6`). |
